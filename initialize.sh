...
install_dependencies() {
  ...
  npm install adm-zip express-session speakeasy qrcode bcryptjs express-rate-limit qrcode-terminal tar bcrypt redis connect-redis hpp
}
...
setup_adpanel_systemd_service() {
  ...
  local entry_file="${panel_root}/index.js"
  ...
  if [ ! -f "$entry_file" ]; then
    ui_error "Expected entry file not found at ${entry_file}."
    return 1
  fi
  if [ ! -f "$prep_script" ]; then
    ui_warn "Expected preparation script not found at ${prep_script}."
    ui_warn "The service will still be created without start.sh integration."
  else
    exec_start_pre_line="ExecStartPre=${prep_script}"
...
[Unit]
Description=ADPanel Node.js Express Service
...
Group=adpanel
${exec_start_pre_line}
ExecStart=/bin/bash -c 'TOTAL_KB=\$(sed -n "s/^MemTotal:[[:space:]]*\\([0-9]*\\).*/\\1/p" /proc/meminfo); V8_FLAGS=""; if [ "\$TOTAL_KB" -le 1048576 ] 2>/dev/null; then V8_FLAGS="--max-old-space-size=300 --optimize-for-size"; echo "[v8] Low-memory mode: \$V8_FLAGS"; fi; exec ${node_bin} \$V8_FLAGS ${entry_file}'
Restart=always
RestartSec=5
...
harden_firewall() {
  ...
  if ! command -v ufw >/dev/null 2>&1; then
    ...
    pkg_install ufw >/dev/null 2>&1 || {
      ui_warn "Could not install ufw. Skipping firewall hardening."
      return 1
    }
  fi

  # Block port 445 (SMB/CIFS) - not used by ADPanel
  $SUDO ufw deny 445/tcp >/dev/null 2>&1 || true
  $SUDO ufw deny 445/udp >/dev/null 2>&1 || true
  ui_success "Port 445 (SMB) blocked."

  # Block MariaDB/MySQL from external access
...
MYSQLEOF
      ui_success "MariaDB/MySQL version comment masked in ${target}"
      mysql_hardened=true
      break
    fi
  done
  if [ "$mysql_hardened" = false ] && [ -f /etc/my.cnf ]; then
    if ! grep -q "version_comment" /etc/my.cnf 2>/dev/null; then
      if grep -q "^\[mysqld\]" /etc/my.cnf 2>/dev/null; then
        $SUDO sed -i '/^\[mysqld\]/a version_comment = ' /etc/my.cnf
      else
        echo -e "\n[mysqld]\nversion_comment = " | $SUDO tee -a /etc/my.cnf >/dev/null
      fi
      ui_success "MariaDB/MySQL version comment masked in /etc/my.cnf"
    fi
  fi
...
-- Schema permissions (explicit CREATE/ALTER + other strictly needed for table/index lifecycle)
...
bootstrap_db_and_create_admin() {
  ...
  if [ -n "$admin_secret" ]; then
    $SUDO node "$script_path" --email "$admin_email" --password-hash "$admin_hash" --admin --secret "$admin_secret"
  else
    $SUDO node "$script_path" --email "$admin_email" --password-hash "$admin_hash" --admin
  fi
...
configure_redis_firewall() {
  ...
  if command -v firewall-cmd >/dev/null 2>&1; then
    $SUDO firewall-cmd --add-port=6379/tcp >/dev/null 2>&1 || true
    $SUDO firewall-cmd --permanent --add-port=6379/tcp >/dev/null 2>&1 || true
    $SUDO firewall-cmd --reload >/dev/null 2>&1 || true
  fi

  if command -v iptables >/dev/null 2>&1; then
    $SUDO iptables -C INPUT -p tcp --dport 6379 -j ACCEPT >/dev/null 2>&1 || \
      $SUDO iptables -A INPUT -p tcp --dport 6379 -j ACCEPT >/dev/null 2>&1 || true
    if command -v systemctl >/dev/null 2>&1; then
...
write_nginx_config() {
  ...
  if [ "$nginx_has_brotli" == "false" ]; then
    if command -v apt-get >/dev/null 2>&1; then
      if $SUDO apt-get install -y libnginx-mod-brotli >/dev/null 2>&1; then
        nginx_has_brotli="true"
        ui_success "nginx brotli module installed."
      fi
    elif command -v yum >/dev/null 2>&1; then
      if $SUDO yum install -y nginx-mod-brotli >/dev/null 2>&1; then
        nginx_has_brotli="true"
        ui_success "nginx brotli module installed."
      fi
    fi
...
EOF
  ...
  if [ -n "$PANEL_DOMAIN" ]; then
    ...
    ui_info "Domain detected. ADPanel will use nginx-managed HTTPS on ports 80/443."
  else
...
EOF
  ...
  read -p "$(ui_prompt "EnterEnter SITE_KEY (leave blank to disable captcha): ")" SITE_KEY
  ...
  # Enterprise security: lock down MariaDB and firewall
  secure_mariadb_binding
  harden_firewall
  mask_service_versions
  ...
  if [ -n "$PANEL_DOMAIN" ] && [ "$NGINX_ENABLED" != "true" ]; then
    ui_error "Domain mode requires nginx, but nginx could not be installed or started."
    return 1
  fi
  ...
  if [ "$ENABLE_HTTPS" == "true" ] && { [ -z "$SSL_CERT_PATH" ] || [ -z "$SSL_KEY_PATH" ]; }; then
    if [ -n "$PANEL_DOMAIN" ]; then
      ui_error "HTTPS certificate setup failed for ${PANEL_DOMAIN}. Domain mode requires a valid certificate."
      return 1
    fi
...
EOF
  ...
  bootstrap_db_and_create_admin "$EMAIL" "$HASH" "$SECRET"
