...
function resolveFaviconAssetUrl(branding) {
  const config = branding && typeof branding === "object" ? branding : loadBrandingConfig();
  return `/auth-assets/favicon${getBrandingAssetVersionSuffix(config)}`;
}
...
function brandingMiddleware(req, res, next) {
  ...
  res.locals.branding = {
    ...
    faviconUrl: resolveFaviconAssetUrl(branding),
    logoUrl: safeLogo 
      ? `/images/${safeLogo}` 
