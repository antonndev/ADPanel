...
async function deleteServerByName(deps, rawName) {
  ...
  try {
    await deps.httpRequestJson(`${baseUrl}/v1/servers/${encodeURIComponent(name)}/stop`, "POST", headers, null, 30_000);
  ...
  try {
    const backupsRes = await deps.httpRequestJson(`${baseUrl}/v1/servers/${encodeURIComponent(name)}/backups`, "GET", headers, null, 10_000);
    if (backupsRes.status === 200 && backupsRes.json?.backups) {
      for (const backup of backupsRes.json.backups) {
        ...
        await deps.httpRequestJson(
          `${baseUrl}/v1/servers/${encodeURIComponent(name)}/backups/${encodeURIComponent(backup.id)}`,
          "DELETE",
          headers,
          null,
          30_000
  ...
  const response = await deps.httpRequestJson(`${baseUrl}/v1/servers/${encodeURIComponent(name)}`, "DELETE", headers, null, 60_000);
