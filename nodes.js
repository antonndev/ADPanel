...
const MAX_CONSOLE_COMMAND_LENGTH = parseInt(process.env.MAX_CONSOLE_COMMAND_LENGTH || "", 10) || 512;

function sanitizeConsoleCommand(rawValue) {
  const raw = String(rawValue || "").replace(/\0/g, "").trim();
  if (!raw) {
    return { ok: false, error: "empty_command" };
  }
  if (raw.length > MAX_CONSOLE_COMMAND_LENGTH) {
    return {
      ok: false,
      error: "command_too_long",
      detail: `Console commands must be ${MAX_CONSOLE_COMMAND_LENGTH} characters or fewer.`,
    };
  }
  if (/[\r\n]/.test(raw)) {
    return {
      ok: false,
      error: "multiline_command_not_allowed",
      detail: "Only one console command can be sent at a time.",
    };
  }
  if (/[\u0001-\u0008\u000B\u000C\u000E-\u001F\u007F]/.test(raw)) {
    return {
      ok: false,
      error: "invalid_command_characters",
      detail: "Console commands cannot include control characters.",
    };
  }
...
async function loadNodesPaginated(page, limit, search) {
  ...
  // MariaDB on this project rejects prepared placeholders inside LIMIT/OFFSET.
  dataSql += ` ORDER BY name ASC LIMIT ${limit} OFFSET ${offset}`;
...
function callNodeApiOnce(fullUrl, method = "GET", headers = {}, body = null, timeoutMs = DEFAULT_NODE_TIMEOUT_MS) {
  return new Promise((resolve) => {
    try {
      ...
      const req = lib.request(fullUrl, { method, headers }, (res) => {
        ...
        res.on("end", () => {
          ...
          try {
            ...
            resolve({ status: res.statusCode, json: null, error: null });
          }
        });
      });
      req.on("timeout", () => {
      ...
      req.on("error", (err) => resolve({ status: 0, json: null, error: err?.message || "request error" }));
      req.setTimeout(timeoutMs);
      if (body) req.write(JSON.stringify(body));
      req.end();
    } catch {
      resolve({ status: 0, json: null, error: "request setup failed" });
    }
  });
}

async function resolveLoopbackNodeBase(node, timeoutMs = 1200) {
  if (!node || isLoopbackAddress(node.address)) return null;
  const candidates = [];

  const primaryProto = node.ssl_enabled ? "https" : "http";
  candidates.push(`${primaryProto}://127.0.0.1:${clampPort(node.api_port || 8080)}`);
  candidates.push(`${primaryProto}://localhost:${clampPort(node.api_port || 8080)}`);
  if (primaryProto === "https") {
    candidates.push(`http://127.0.0.1:${clampPort(node.api_port || 8080)}`);
    candidates.push(`http://localhost:${clampPort(node.api_port || 8080)}`);
  }

  const expectedUuid = String(node.uuid || node.id || "").trim();
  for (const base of candidates) {
    const probe = await callNodeApiOnce(`${base}/health`, "GET", {}, null, timeoutMs);
    if (probe.status !== 200 || !probe.json) continue;
    const gotUuid = String(probe.json.uuid || "").trim();
    if (expectedUuid && gotUuid && gotUuid === expectedUuid) {
      return base;
    }
  }
  return null;
...
async function callNodeApi(node, pathSuffix, method = "GET", body = null, timeoutMs = DEFAULT_NODE_TIMEOUT_MS) {
  const base = buildNodeBaseUrl(node.address, node.api_port, node.ssl_enabled);
  if (!base) return { status: 0, json: null, error: "invalid node base url" };

  const safeSuffix = String(pathSuffix || "").startsWith("/")
    ? String(pathSuffix)
    : `/${String(pathSuffix || "")}`;
  ...
  const primaryRes = await callNodeApiOnce(`${base}${safeSuffix}`, method, headers, body, timeoutMs);
  if (primaryRes.status !== 0) return primaryRes;

  const fallbackBase = await resolveLoopbackNodeBase(node, Math.min(timeoutMs, 1500));
  if (!fallbackBase || fallbackBase === base) return primaryRes;

  const fallbackRes = await callNodeApiOnce(`${fallbackBase}${safeSuffix}`, method, headers, body, timeoutMs);
  if (fallbackRes.status !== 0) {
    return { ...fallbackRes, via: "loopback-fallback" };
  }
  return primaryRes;
}
...
app.post("/api/nodes/:id/servers/create", async (req, res) => {
  ...
  const requestedHostPort = req.body?.hostPort;
  const hostPort = requestedHostPort == null || String(requestedHostPort).trim() === ""
    ? 25565
    : validatePortInput(requestedHostPort);
  const autoStart = !!req.body?.autoStart;
  if (!name) return res.status(400).json({ error: "missing_name" });
  if (!hostPort) return res.status(400).json({ error: "invalid_host_port" });
...
setInterval(async () => {
  try {
    ...
    const batch = await db.query(
      `SELECT * FROM nodes ORDER BY id ASC LIMIT ${HC_BATCH_SIZE} OFFSET ${_hcOffset}`
    );
...
async function startPayloadFor(name) {
  const srv = await findServerByNameOrId(name);
  // Node daemon /start accepts only hostPort + env; keep payload strict to avoid 400 invalid request body.
  const chosenPort = validatePortInput(srv?.hostPort ?? srv?.port ?? srv?.server_port);
  const payload = {};
  if (chosenPort != null) payload.hostPort = chosenPort;
...
async function seedRuntimeOnNode(node, name) {
  try {
    const srv = await findServerByNameOrId(name);
    ...
    if (srv && tpl && !isMinecraft && isRuntimeTemplate) {
      const runtimePayload = {
        ...
        port: srv.hostPort || srv.port || srv.server_port || null,
...
async function performPanelServerAction(req, serverName, cmdRaw) {
  const targetServer = String(serverName || "").trim();
  const cmdValue = String(cmdRaw || "").toLowerCase();
  const cmd = (cmdValue === "run") ? "start" : cmdValue;

  const ctx = await remoteContext(targetServer);
  if (!ctx.exists) return { status: 404, body: { error: "server not found" } };
  if (!ctx.remote || !ctx.node) return { status: 400, body: { error: "not_remote" } };

  const accessCheck = await checkServerAccess(req, targetServer);
  if (!accessCheck.ok) return { status: accessCheck.status, body: { error: accessCheck.error } };

  let requiredPerms = ["server_start"];
  if (cmd === "start") requiredPerms = ["server_start"];
  else if (cmd === "restart") requiredPerms = ["server_stop", "server_start"];
  else if (cmd === "stop" || cmd === "kill") requiredPerms = ["server_stop"];
  else if (cmd === "status") requiredPerms = [];
  else return { status: 400, body: { error: "invalid_cmd" } };

  for (const requiredPerm of requiredPerms) {
    if (!(await checkUserPerm(req, targetServer, requiredPerm))) {
      return { status: 403, body: { error: "permission denied", permission: requiredPerm } };
    }
  }
  ...
  if (cmd === "start") {
    try {
      if (ctx.node) await seedRuntimeOnNode(ctx.node, targetServer);
    } catch (_) { }
    path = `/v1/servers/${encodeURIComponent(targetServer)}/start?wait=false`;
    payload = await startPayloadFor(targetServer);
  } else if (cmd === "stop") {
    path = `/v1/servers/${encodeURIComponent(targetServer)}/stop`;
  } else if (cmd === "kill") {
    path = `/v1/servers/${encodeURIComponent(targetServer)}/kill`;
  } else if (cmd === "restart") {
    path = `/v1/servers/${encodeURIComponent(targetServer)}/restart`;
  } else if (cmd === "status") {
    method = "GET";
    path = `/v1/servers/${encodeURIComponent(targetServer)}`;
  }
  ...
  if (cmd !== "status") {
    ...
    const userEmail = req.session?.user || "unknown";
    recordActivity(targetServer, actionName, null, userEmail, getRequestIp(req));
  }

  const timeoutMs = cmd === "start" ? LONG_NODE_TIMEOUT_MS : DEFAULT_NODE_TIMEOUT_MS;
  let result = await callNodeApi(ctx.node, path, method, payload, timeoutMs);
  if (result.status === 0 && cmd === "start") {
    result = await callNodeApi(ctx.node, path, method, payload, timeoutMs);
  }
  const { status, json, error } = result;
  if ((status === 200 || status === 202) && json && (json.ok === true || json.ok === undefined)) {
    return { status: 200, body: json || { ok: true } };
  }
  const detail = (json && (json.detail || json.error || json.message)) || error || `HTTP ${status}`;
  console.warn(`[nodes-action] ${targetServer} ${cmd} failed: status=${status} detail=${String(detail)}`);
  return {
    status: 502,
    body: {
...
async function performPanelConsoleCommand(req, serverName, rawCommand) {
  const targetServer = String(serverName || "").trim();
  const ctx = await remoteContext(targetServer);
  if (!ctx.exists) return { status: 404, body: { error: "server not found" } };
  if (!ctx.remote || !ctx.node) return { status: 400, body: { error: "not_remote" } };
  ...
  if (!(await checkUserPerm(req, targetServer, "console_write"))) {
    return { status: 403, body: { error: "permission denied", permission: "console_write" } };
  }

  const commandCheck = sanitizeConsoleCommand(rawCommand);
  if (!commandCheck.ok) {
    return { status: 400, body: { error: commandCheck.error, detail: commandCheck.detail || null } };
  }
  const command = commandCheck.command;

  const userEmail = req.session?.user || "unknown";
  recordActivity(targetServer, "console_command", { command }, userEmail, getRequestIp(req));

  const { status, json } = await callNodeApi(
    ctx.node,
    `/v1/servers/${encodeURIComponent(targetServer)}/command`,
    "POST",
    { command }
  );
  if (status === 200 && json && json.ok) return { status: 200, body: json };
  return {
    status: 502,
    body: {
      error: "node_command_failed",
      status,
      detail: (json && (json.detail || json.error)) || `status ${status}`,
    },
  };
}

async function performPanelListBackups(req, serverName) {
  const targetServer = String(serverName || "").trim();
  const accessCheck = await checkServerAccess(req, targetServer);
  if (!accessCheck.ok) return { status: accessCheck.status, body: { error: accessCheck.error } };

  if (!(await checkUserPerm(req, targetServer, "backups_view"))) {
    return { status: 403, body: { error: "forbidden", detail: "You do not have permission to view backups" } };
  }

  const ctx = await remoteContext(targetServer);
  if (!ctx.exists) return { status: 404, body: { error: "server not found", detail: "Server does not exist in database" } };
  if (!ctx.remote || !ctx.node) return { status: 400, body: { error: "not_remote", detail: "Server is not associated with a node" } };

  const url = nodeUrl(ctx.node, `/v1/servers/${encodeURIComponent(targetServer)}/backups`);
  const { status, json, text } = await httpJson(url, { method: "GET", headers: nodeHeaders(ctx.node), timeoutMs: 15000 });

  if (status === 0) {
    return { status: 502, body: { error: "node_unreachable", detail: "Could not connect to node agent" } };
  }
  if (status !== 200 || !json || !json.ok) {
    return { status: 502, body: { error: "node_backup_list_failed", detail: json?.error || text || `status ${status}` } };
  }
  return { status: 200, body: { ok: true, backups: json.backups || [] } };
}

async function performPanelCreateBackup(req, serverName, backupName, description) {
  const targetServer = String(serverName || "").trim();
  const accessCheck = await checkServerAccess(req, targetServer);
  if (!accessCheck.ok) return { status: accessCheck.status, body: { error: accessCheck.error } };

  if (!(await checkUserPerm(req, targetServer, "backups_create"))) {
    return { status: 403, body: { error: "forbidden", detail: "You do not have permission to create backups" } };
  }
  ...
  if (status === 507 && json?.error === "disk_limit_exceeded") {
    return { status: 507, body: json };
  }
  if (status !== 200 || !json || !json.ok) {
    return { status: 502, body: { error: "node_backup_create_failed", detail: json?.error || text || `status ${status}` } };
  }

  const userEmail = req.session?.user || "unknown";
  const userIp = getRequestIp(req);
  recordActivity(targetServer, "backup_create", { backupId: json.backup?.id, name: json.backup?.name }, userEmail, userIp);

  return { status: 200, body: { ok: true, backup: json.backup } };
}

async function performPanelRestoreBackup(req, serverName, backupIdRaw, deleteOldFiles) {
  const targetServer = String(serverName || "").trim();
  const accessCheck = await checkServerAccess(req, targetServer);
  if (!accessCheck.ok) return { status: accessCheck.status, body: { error: accessCheck.error } };

  if (!(await checkUserPerm(req, targetServer, "backups_create"))) {
    return { status: 403, body: { error: "forbidden", detail: "You do not have permission to restore backups" } };
  }
  ...
  if (!backupId) return { status: 400, body: { error: "missing backup id" } };

  const url = nodeUrl(ctx.node, `/v1/servers/${encodeURIComponent(targetServer)}/backups/${encodeURIComponent(backupId)}/restore`);
  const { status, json, text } = await httpJson(url, {
    method: "POST",
    headers: Object.assign({ "Content-Type": "application/json" }, nodeHeaders(ctx.node)),
  ...
  if (status !== 200 || !json || !json.ok) {
    return { status: 502, body: { error: "node_backup_restore_failed", detail: json?.error || text || `status ${status}` } };
  }

  const userEmail = req.session?.user || "unknown";
  const userIp = getRequestIp(req);
  recordActivity(targetServer, "backup_restore", { backupId, deleteOldFiles: !!deleteOldFiles }, userEmail, userIp);

  return { status: 200, body: { ok: true } };
}

async function performPanelDeleteBackup(req, serverName, backupIdRaw) {
  const targetServer = String(serverName || "").trim();
  const accessCheck = await checkServerAccess(req, targetServer);
  if (!accessCheck.ok) return { status: accessCheck.status, body: { error: accessCheck.error } };

  if (!(await checkUserPerm(req, targetServer, "backups_delete"))) {
    return { status: 403, body: { error: "forbidden", detail: "You do not have permission to delete backups" } };
  }
  ...
  const url = nodeUrl(ctx.node, `/v1/servers/${encodeURIComponent(targetServer)}/backups/${encodeURIComponent(backupId)}`);
  const { status, json, text } = await httpJson(url, {
    method: "DELETE",
  ...
  if (status !== 200 || !json || !json.ok) {
    return { status: 502, body: { error: "node_backup_delete_failed", detail: json?.error || text || `status ${status}` } };
  }

  const userEmail = req.session?.user || "unknown";
  const userIp = getRequestIp(req);
  recordActivity(targetServer, "backup_delete", { backupId }, userEmail, userIp);

  return { status: 200, body: { ok: true } };
}
app.post("/api/nodes/server/:name/action", async (req, res) => {
  try {
    const result = await performPanelServerAction(req, req.params.name, req.body && (req.body.cmd || req.body.action));
...
app.post("/api/nodes/server/:name/command", async (req, res) => {
  try {
    const result = await performPanelConsoleCommand(req, req.params.name, req.body && req.body.command);
    return res.status(result.status).json(result.body);
  } catch (e) {
    return res.status(500).json({ error: "bridge_failed", detail: e && e.message });
  }
...
app.post("/api/nodes/:id/server/action", async (req, res) => {
  try {
    ...
    const result = await performPanelServerAction(req, name, req.body && req.body.cmd);
    return res.status(result.status).json(result.body);
  } catch (e) {
    return res.status(500).json({ error: "bridge_failed", detail: e && e.message });
  }
...
app.post("/api/nodes/:id/server/command", async (req, res) => {
  try {
    ...
    const result = await performPanelConsoleCommand(req, name, req.body && req.body.command);
    return res.status(result.status).json(result.body);
  } catch (e) {
    return res.status(500).json({ error: "bridge_failed", detail: e && e.message });
  }
...
app.get("/api/nodes/server/:name/backups", async (req, res) => {
  try {
    const result = await performPanelListBackups(req, req.params.name);
    return res.status(result.status).json(result.body);
  } catch (e) {
...
app.post("/api/nodes/server/:name/backups", async (req, res) => {
  try {
    const result = await performPanelCreateBackup(req, req.params.name, req.body?.name, req.body?.description);
    return res.status(result.status).json(result.body);
  } catch (e) {
...
app.post("/api/nodes/server/:name/backups/:backupId/restore", async (req, res) => {
  try {
    const result = await performPanelRestoreBackup(req, req.params.name, req.params.backupId, req.body?.deleteOldFiles);
    return res.status(result.status).json(result.body);
  } catch (e) {
...
app.delete("/api/nodes/server/:name/backups/:backupId", async (req, res) => {
  try {
    const result = await performPanelDeleteBackup(req, req.params.name, req.params.backupId);
    return res.status(result.status).json(result.body);
  } catch (e) {
...
app.get("/api/nodes/server/:name/logs", async (req, res) => {
  try {
    ...
    const base = buildNodeBaseUrl(node.address, node.api_port, node.ssl_enabled);
    const tailRaw = String(req.query?.tail || "").trim();
    let tailQuery = "";
    if (/^\d+$/.test(tailRaw)) {
      const tail = Math.max(0, Math.min(10000, parseInt(tailRaw, 10)));
      tailQuery = `?tail=${encodeURIComponent(String(tail))}`;
    }
    const fullUrl = `${base}/v1/servers/${encodeURIComponent(serverName)}/logs${tailQuery}`;
    const isHttps = fullUrl.startsWith("https:");
    const lib = isHttps ? https : http;
    const nreq = lib.request(fullUrl, {
      ...
      if (nres.statusCode !== 200) {
        const chunks = [];
        nres.on("data", (chunk) => {
          if (Buffer.concat(chunks).length < 16 * 1024) chunks.push(Buffer.from(chunk));
        });
        nres.on("end", () => {
          const detail = Buffer.concat(chunks).toString("utf8").slice(0, 2000);
          if (!res.headersSent) {
            return res.status(nres.statusCode || 502).json({ error: "node_logs_failed", detail: detail || `status ${nres.statusCode || 502}` });
          }
          try { res.end(); } catch { }
        });
        return;
...
module.exports.callNodeApi = callNodeApi;
module.exports.resolveNodeForServer = resolveNodeForServer;
module.exports.recordActivity = recordActivity;
module.exports.seedRuntimeOnNode = seedRuntimeOnNode;
module.exports.setWebhookDispatcher = setWebhookDispatcher;
module.exports.setPermissionCheckers = setPermissionCheckers;
module.exports.startPayloadFor = startPayloadFor;
module.exports.sanitizeConsoleCommand = sanitizeConsoleCommand;
module.exports.performPanelServerAction = performPanelServerAction;
module.exports.performPanelConsoleCommand = performPanelConsoleCommand;
module.exports.performPanelListBackups = performPanelListBackups;
module.exports.performPanelCreateBackup = performPanelCreateBackup;
module.exports.performPanelRestoreBackup = performPanelRestoreBackup;
module.exports.performPanelDeleteBackup = performPanelDeleteBackup;
